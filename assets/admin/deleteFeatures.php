...
if(!empty($_POST))
{
    ...
    $db = Database::connect();
    $statement = $db->prepare("DELETE FROM featurs WHERE id = ?");
...
<div class="container">
    <h1><strong class="text-center">Delete Features</strong></h1>

    <div class="row">
        <br>
        <form class="form" action="deleteFeatures.php" role="form" method="post">
            <input type="hidden" name="id" value="<?php echo $id;?>"/>
            <p class="alert alert-warning">Are you sur that you want to delete ?</p>
            <div class="form-actions">
                <button type="submit" class="btn btn-warning">Yes</button>
                <a class="btn btn-default" href="indexFeatures.php">No</a>
            </div>
        </form>
    </div>
</div>
